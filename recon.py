#!/usr/bin/env python3
"""
ZephyrusRecon Kali Linux - Recon pipeline with Discord real-time updates + colored terminal + partial nuclei streaming
Author: Zephyrus
Version: v1.3-kali
"""
...
REQUIRED_TOOLS = {
    ...
    "httpx-toolkit": "github.com/projectdiscovery/httpx/cmd/httpx@latest",  # httpx-toolkit untuk Kali
...
def print_banner():
    try:
        ...
        print("                     v1.3-kali | Author: Zephyrus\n")
    except ImportError:
        print("ZephyrusRecon v1.3-kali | Author: Zephyrus\n")
...
def ensure_tools_auto():
    ...
    if need and not ensure_go(): failed["go"]="go not installed"
    for t in need:
        if go_install(REQUIRED_TOOLS[t]): installed.append(t)
        else: failed[t] = "go install failed"
...
def run_httpx_toolkit_on_list(list_lines):
    # Untuk Kali Linux, gunakan httpx-toolkit (alias key di REQUIRED_TOOLS)
    alive = strip_scheme(list_lines)
    return 0, "\n".join(alive), ""
...
# ----- Nuclei streaming -----
def run_nuclei_stream(urls_file, out_file, severity="critical,high,medium", periodic_upload=True):
    cmd=["nuclei","-l",urls_file,"-severity",severity]
    proc=subprocess.Popen(cmd,stdout=subprocess.PIPE,stderr=subprocess.PIPE,text=True,bufsize=1)
    buffer_lines=[]; last_upload=time.time()
    with open(out_file,"w",encoding="utf-8") as fout:
        for line in proc.stdout:
            ...
            if periodic_upload and (time.time()-last_upload)>=900 and buffer_lines:
...
def send_nuclei_summary(out_file, workdir, target):
    ...
    for l in lines:
        if "[" in l and "]" in l: severity_counts[l.split("[")[-1].split("]")[0]]+=1
    summary=(
        f"🏁 SCAN COMPLETED\nWorkdir: {workdir}\nTarget: {target}\n"
        f"Targets scanned: {len(lines)}\nNuclei findings: {len(lines)}\n"
        f"Severity\n"
...
def pipeline_for_domain(domain, workdir:Path, args):
    workdir.mkdir(parents=True, exist_ok=True); ts=datetime.now().strftime("%Y%m%d_%H%M%S")
    out_sub=workdir/f"subfinder_{ts}.txt"; out_way=workdir/f"wayback_{ts}.txt"
    out_kat=workdir/f"katana_{ts}.txt"; out_nuc=workdir/f"nuclei_output_{ts}.txt"
    ...
    run_subfinder(domain,str(out_sub)); subs=read_lines(str(out_sub))
    ...
    rc, stdout, _ = run_httpx_toolkit_on_list(subs); alive=stdout.splitlines()
    ...
    run_wayback(domain,str(out_way)); wayback=read_lines(str(out_way))
    ...
    run_katana(str(tmp_hosts),str(out_kat)); kat=read_lines(str(out_kat))
    combined.update(kat); cprint(f"Katana URLs collected: {len(kat)}",Fore.GREEN)

    # Nuclei input
    if kat: nuclei_input=kat
    elif wayback: nuclei_input=wayback
    else: nuclei_input=alive
    tmp_list=workdir/f"nuclei_list_{ts}.txt"
    with open(tmp_list,"w") as f: f.writelines(u+"\n" for u in nuclei_input)
    ...
    if choice=="1":
        ...
        with ThreadPoolExecutor(max_workers=args.concurrency) as ex:
            ...
            for fut in as_completed(futures):
                ...
                except Exception as e: send_discord_embed("⚠️ Scan error",f"{d} error: {e}")
