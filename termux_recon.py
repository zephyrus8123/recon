#!/usr/bin/env python3
"""
ZephyrusRecon - Recon pipeline with Discord real-time updates + colored terminal + partial nuclei streaming
Author: Zephyrus
Version: v1.3
"""
...
def print_banner():
    try:
        ...
        print("                     v1.3 | Author: Zephyrus\n")
    except ImportError:
        print("ZephyrusRecon v1.3 | Author: Zephyrus\n")
...
def ensure_go():
    ...
    cprint("[!] 'go' not found. Attempting install...", Fore.RED)
    if which("apt-get"):
...
def pipeline_for_domain(domain, workdir:Path, args):
    ...
    rc, stdout, _ = run_httpx_on_list(subs); alive=stdout.splitlines()
...
def main():
    print_banner()
    parser=argparse.ArgumentParser(description="ZephyrusRecon - Recon pipeline")
