...
- HTTPX output clean (no scheme, no [SUCCESS]/[FAILED])
- Nuclei input only valid URLs
- Discord embeds for all steps
- Partial nuclei upload + final upload
"""
...
def clean_httpx_output(lines):
    cleaned=[]
    for l in lines:
        l=l.strip().split()[0]  # hapus [SUCCESS]/[FAILED]
        if l:
            cleaned.append(l)
    return strip_scheme(cleaned)

# ----- External tool wrappers -----
def run_subfinder(domain, out_path):
    return run(["subfinder","-d",domain,"-o",out_path], timeout=600)
...
def run_httpx_on_list(list_lines):
    ...
    try:
        ...
        alive = clean_httpx_output(stdout.splitlines())
        return 0,"\n".join(alive), stderr
    except Exception as e:
        return 1,"",str(e)

def run_wayback(domain, out_path):
    return run(["bash","-lc",f"echo {domain} | waybackurls 2>/dev/null | tee {out_path}"], timeout=300, shell=True)

def run_katana(hosts_file,out_file):
    return run(["katana","-l",hosts_file,"-depth","2","-o",out_file], timeout=1800)
...
def run_nuclei_stream(urls_file,out_file,severity="critical,high,medium",batch_size=5,batch_timeout=900,periodic_upload=True):
    proc=subprocess.Popen(["nuclei","-l",urls_file,"-severity",severity], stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True, bufsize=1)
    buffer_lines=[]
...
def send_partial_findings(lines):
    ...
    send_discord_embed("Partial Nuclei Findings", description=summary, color=0xf1c40f)
...
def pipeline_for_domain(domain, workdir:Path, args):
    ...
    send_discord_embed("🚀 RECON SCAN STARTED", f"Target: {domain}\nSteps: subfinder → httpx → wayback → katana → filter → nuclei", color=0x3498db)
    ...
    if combined:
        ...
        nuclei_source = alive_clean

    if not nuclei_source:
        send_discord_embed("⚠️ Nuclei","No URLs to scan", color=0xe74c3c)
    ...
    send_discord_embed("🏁 Scan Completed", f"Scan finished for {domain}", color=0x2ecc71)
    return {"status":"done"}
